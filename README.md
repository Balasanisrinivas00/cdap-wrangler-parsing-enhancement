...
More [here](wrangler-docs/upcoming-features.md) on upcoming features.
  ...
  * **Byte Size and Time Duration Parsers**: CDAP Wrangler now includes native support for parsing and utilizing data with byte size units (B, KB, MB, GB, TB, PB) and time duration units (ms, s, m, h, d). These new parsers enable easier handling of data sizes and time intervals without needing complex multi-step recipes.
    * The `ByteSize` parser recognizes values like "10KB", "1.5MB", and "2GB", converting them to canonical byte units.
    * The `TimeDuration` parser recognizes values like "100ms", "5s", "10m", and "2h", converting them to canonical time units.
    * A new `aggregate-stats` directive utilizes these parsers to calculate statistics (totals or averages) on size and time values with appropriate unit conversions.
...
| **Aggregation**                                                        |                                                                  |
| [Aggregate Stats](wrangler-docs/directives/aggregate-stats.md)                  | Aggregates byte size and time duration columns with proper units |
| **Output Formatters**                                                  |                                                                  |
